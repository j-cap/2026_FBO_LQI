...
\begin{equation}
    \alpha_{ij}=\frac{(1-\hat p_{ij}^{\mathrm{risk}})^\gamma}{\sum_{\ell\in\mathcal N_i}(1-\hat p_{i\ell}^{\mathrm{risk}})^\gamma}.
\end{equation}
...
\begin{equation}
    \hat p_i^{\mathrm{stab}}(\xi)=\frac{1}{M_u}\sum_{m=1}^{M_u}\mathbb I\!\left[\rho(A_{\mathrm{cl}}(\theta_i^{(m)},K_i(\xi)))<1-\varepsilon_\rho\right].
\end{equation}
...
Use the existing heterogeneous lateral-vehicle fleet from~\cite{weber2026ifac}. Each client has the two-state lateral variables $x=[\beta,r]^\top$, steering input $u=\delta$, and yaw-rate tracking output. The default fleet contains three latent regimes representing low-speed, payload, and high-speed operation with client-level parameter scatter.

Two plant conditions should be reported:
\begin{enumerate}
\item \textbf{Phase 1A -- matched model:} the true client and identified controller model share the linear bicycle-model structure. Differences arise from client parameters and finite identification uncertainty.
\item \textbf{Phase 1B -- structural mismatch:} the true plant uses smooth saturating tire forces, while RLS identification and LQI synthesis retain the linear two-state model. This creates a controlled model-reality gap without changing the federated identification interface.
\end{enumerate}
Data must be split into disjoint identification, BO-calibration, and held-out test episodes.

\subsection{Phase-1 mechanism study}
Before evaluating FBO, establish the mechanism that motivates selective transfer. On a common Sobol set of LQI weights, compute pairwise uncertainty-aware dynamics distance $\distdyn_{ij}$, pairwise controller-transfer loss $L_{i\leftarrow j}$ from~\eqref{eq:transfer_loss}, whole-landscape rank distance as a secondary diagnostic, and global, true-regime, estimated-cluster, and individual held-out calibration performance. The primary figure is $\distdyn_{ij}$ versus $L_{i\leftarrow j}$ with fleet-level bootstrap confidence intervals. A second analysis bins $\distdyn_{ij}$ and estimates
\begin{equation}
    \Pr(L_{i\leftarrow j}>\tau_L\mid \distdyn_{ij}\in\text{bin}),
\end{equation}
which directly motivates the transfer-risk map.
...
\end{tabular}
\end{table}
Budget comparisons must be matched by the expensive quantity: the number of local closed-loop evaluations. Server rounds alone are not a fair sample-efficiency metric when different methods evaluate different numbers of clients.
...
\todo{Insert full-scale Phase 1A and Phase 1B headline figure and update numerical values. The main argument should be mechanistic rather than only statistical: global and hard-cluster tuning can perform similarly in fleet-average RMSE while pairwise transfer still contains substantial tail risk. This motivates selective transfer even when hard specialization does not improve the mean.}

\subsection{Does dynamics-aware FBO reduce local experiments?}
\todo{Primary DA-FBO result. Plot best held-out RMSE / normalized regret versus local controller evaluations for M1--M7. The desired result is that DA-FBO improves early sample efficiency over independent BO and avoids the negative-transfer tail of global FTS.}

\subsection{Safety-screening effectiveness}
\todo{Compare candidate proposals, pre-evaluation rejections, actual unsafe rollouts, false rejections, and final tracking performance. Separate transfer gating from target-local plant screening so that their roles remain visible.}

\subsection{Matched versus mismatched plants}
\todo{Use Phase 1A as a negative-control / low-mismatch setting and Phase 1B as the main realistic case. Discuss whether the benefit of dynamics-aware transfer grows when model-based synthesis is imperfect.}

\subsection{Limitations}
The first iteration should explicitly discuss: the dynamics distance~\eqref{eq:dyn_distance} is symmetric while empirical controller transfer can be directional; the risk calibration $g_{\tau_L}$ may be fleet- and task-dependent and therefore requires a development fleet or online adaptation; raw-data locality does not imply differential privacy; Monte Carlo screening from identification covariance is not a formal robust-stability guarantee unless the uncertainty set and certificate are strengthened accordingly; the study uses a low-dimensional LQI calibration space, which is appropriate for BO but does not directly establish scalability to high-dimensional controller architectures; and the vehicle validation remains simulation based until real fleet data or hardware experiments are added.

\section{Conclusion}
\label{sec:conclusion}
This paper studies federated controller calibration for fleets in which clients share a control task but not identical dynamics. The proposed framework uses uncertain local system identifications to decide how Bayesian-optimization experience should transfer between clients. Rather than assigning one calibration per hard cluster, dynamics-aware federated BO treats identified similarity as a continuous trust signal and combines it with target-local uncertainty-aware controller screening. This architecture preserves model-based LQI synthesis while using distributed closed-loop experiments only for the low-dimensional calibration that the identified model cannot determine reliably.
...
\item \textbf{Experimental priority:} finish full-scale Phase 1B, then implement M1/M2/M4 before adding the risk-calibrated and safety-screened variants. The paper can survive without a sophisticated risk calibrator if direct uncertainty-aware similarity already produces the desired sample-efficiency/tail-risk behavior.
\end{itemize}
